...
namespace Sinx.Net.Http.Headers
{
	...
	/// </summary>
	/// <remarks>
	///		1. 对 ContentType 的处理, 代表 ContentType 的值
	/// </remarks>
    public class MediaTypeHeaderValue
    {
        ...
        public IList<NameValueHeaderValue> Parameters => 
			_parameters ?? (_parameters = IsReadOnly ? ObjectCollection<NameValueHeaderValue>.EmptyReadOnlyCollection : 
				new ObjectCollection<NameValueHeaderValue>());
        ...
        public string Type => _mediaType.Substring(0, _mediaType.IndexOf('/'));

	    public string SubType => _mediaType.Substring(_mediaType.IndexOf('/') + 1);
        ...
        /// SubType = "*"
        /// </summary>
        public bool MatchesAllSubTypes => string.Compare(_mediaType, _mediaType.IndexOf('/') + 1, "*", 0, 1, StringComparison.Ordinal) == 0;
        ...
        {
	        var other = new MediaTypeHeaderValue {_mediaType = _mediaType};
        ...
        private static int GetMediaTypeLength(string input, int startIndex, out MediaTypeHeaderValue parsedValue)
        {
            ...
            {
	            mediaTypeHeader = new MediaTypeHeaderValue {_mediaType = mediaType};
        ...
        {
            Contract.Requires((!string.IsNullOrEmpty(input)) && (startIndex < input.Length));
