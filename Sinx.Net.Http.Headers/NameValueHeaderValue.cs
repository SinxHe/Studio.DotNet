...
{
    // According to the RFC, in places where a "parameter" is required, the value is mandatory(强制的)
    // (e.g. Media-Type, Accept). However, we don't introduce a dedicated(专用的) type for it. So NameValueHeaderValue supports
